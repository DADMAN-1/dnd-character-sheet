...
export default function AttacksTab({ actor, characterId }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [attacksRes, noteRes] = await Promise.allSettled([
        actor.getPhysicalAttacksByCharacter(characterId) as unknown as Promise<
          [bigint, CharacterPhysicalAttack][]
        >,
        actor.getTabNote(characterId, "attacks"),
      ]);
      if (attacksRes.status === "fulfilled")
        setAttacks(attacksRes.value.map(([id, a]) => ({ id, ...a })));
  ...
  const handleUse = async (a: AttackWithId) => {
    await actor.updateCharacterPhysicalAttack(a.id, {
      ...a,
  ...
  const handleReset = async (a: AttackWithId) => {
    await actor.updateCharacterPhysicalAttack(a.id, { ...a, timesUsed: 0n });
    await load();
  };

  const saveNote = async () => {
    setSavingNote(true);
    await actor.saveTabNote(characterId, "attacks", tabNote);
    setSavingNote(false);
  };
  ...
  const filteredAttacks = search
    ? attacks.filter(
        (a) =>
          a.name.toLowerCase().includes(search.toLowerCase()) ||
          a.damageType.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div data-ocid="attacks.section">
      ...
      <input
        className="ds-input"
        placeholder="Search attacks by name or type..."
        value={search}
